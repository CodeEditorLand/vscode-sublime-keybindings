...
async function showPrompt(): Promise<void> {
	const answer: string | undefined =
		await vscode.window.showInformationMessage(
			"Would you like to customize VS Code to behave more like Sublime Text?",
			"Yes",
			"No"
		);
...
async function start(): Promise<void> {
	...
	if (categorizedSettings) {
		if (
			...
			categorizedSettings.defaultSettings.length
		) {
			...
			vscode.window.showInformationMessage(
				"Nothing to import. All settings have already been imported"
			);
		}
	}
}
...
async function getSublimeFolderPath(): Promise<string | undefined> {
	...
	return await browsePrompt(
		`No Sublime settings file found at the default location: ${path.join(
			sublimeFolderFinder.getOSDefaultPaths()[0],
			sublimeFolderFinder.sublimeSettingsFilename
		)} `
	);
}
		...
		if (sublimeSettingsFiles && sublimeSettingsFiles.length) {
			...
			if (isValidFilePath) {
				...
				vscode.window.showErrorMessage(
					`Could not find ${sublimeFolderFinder.sublimeSettingsFilename} at ${sublimeSettingsFiles[0].fsPath} `
				);
			}
		}
	}
...
function validate(settingsFilePath: string): boolean {
	return settingsFilePath.endsWith(
		sublimeFolderFinder.sublimeSettingsFilename
	);
}
...
async function getSettings(
	...
	const settings: CategorizedSettings | undefined =
...
): Promise<VscodeSetting[]> {
	// showing mapped & default settings
	const pickedItems = await vscode.window.showQuickPick(
		[
			...settings.mappedSettings.map((ms) =>
				setting2QuickPickItem(ms.vscode, ms.sublime.name)
			),
			...settings.defaultSettings.map((s) => setting2QuickPickItem(s)),
		],
		{ canPickMany: true, ignoreFocusOut: true }
	);
	// converting all selected entries to VscodeSettings
	return pickedItems
		? pickedItems.map((pickItem) =>
				pickItem.setting instanceof MappedSetting
					? pickItem.setting.vscode
					: pickItem.setting
		  )
		: [];
}

function setting2QuickPickItem(
	setting: VscodeSetting,
	sublimeName?: string
): ISettingsQuickPickItem {
	const icons = {
		exclamationPoint: "$(issue-opened)",
		arrowRight: "$(arrow-right)",
	}; // stored in var because auto-format adds spaces to hypens
	return {
