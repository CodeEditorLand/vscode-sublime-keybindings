...
export class Mapper {
	constructor(
		private settings?: IMapperSettings,
		private mockConfig?: any,
	) {}

	public async getMappedSettings(
	...
	private async getSettings(): Promise<IMapperSettings> {
		if (!this.settings) {
			...
			const [mappingsFile, defaultsFile] = await Promise.all([
				readFileAsync(
					resolve(__dirname, "..", "settings/mappings.json"),
					"utf-8",
				),
				readFileAsync(
					resolve(__dirname, "..", "settings/defaults.json"),
					"utf-8",
				),
			]);
			this.settings = {
				...
				defaults: (rjson.parse(defaultsFile) as [[string, any]]).map(
					(setting) => new VscodeSetting(setting[0], setting[1]),
				),
			};
		}
			...
			if (vscodeSetting) {
				...
				if (configTest.alreadyExists) {
					...
					if (configTest.existingValue) {
						mappedSetting.vscode.markAsOverride(
	...
	private checkWithExistingSettings(
		...
		if (info && info.globalValue !== undefined) {
			if (info.globalValue === vscodeSetting.value) {
				...
				returnVal.existingValue =
					returnVal.existingValue === null
						? ""
	...
	private appendDefaultSublimeSettings(
		...
		defaultSettings: VscodeSetting[],
		config: vscode.WorkspaceConfiguration,
	): CategorizedSettings {
		const mappedAndExisting: MappedSetting[] = [
			...settings.mappedSettings,
			...settings.alreadyExisting,
		];
		...
		const uniqueDefaultSettings = mappedAndExisting.length
			? defaultSettings.filter(
					(defaultSetting) =>
						!mappedAndExisting.find(
							(mappedSetting) =>
								mappedSetting.vscode.name ===
								defaultSetting.name,
						),
				)
			: defaultSettings;
		...
		uniqueDefaultSettings.forEach((defaultSetting) => {
			...
			if (configTest.alreadyExists) {
				settings.alreadyExisting.push(
					new MappedSetting(
						{ name: vscode.l10n.t("Default Setting"), value: "" },
						defaultSetting,
					),
				);
			} else {
	...
	private mapSetting(
		sublimeSetting: ISetting,
		mappedValue: any,
	): VscodeSetting | undefined {
			...
			if (typeof mappedValue === "string") {
				...
				if (!obj) {
					vscode.window.showErrorMessage(
						vscode.l10n.t({
							message:
								"Failed to parse setting: '{0}: {1}'. Please check if it contains syntax errors",
							args: [sublimeSetting.name, sublimeSetting.value],
							comment: [
								"{0} is the name of the setting, {1} is the value of the setting",
							],
						}),
					);
